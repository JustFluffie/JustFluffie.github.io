...
export const messageAiActions = {
  ...
  async summarizeMessages(charId, options) {
    ...
    let systemPrompt = `# 任务
你是一个AI角色的记忆核心生成器。你的任务是根据以下聊天记录，为角色“${character.name}”生成一段简短的、以第一人称视角（“我”）书写的核心记忆。

# 风格要求
这份记忆应该像一本**清晰、简洁的个人日志**。它以事实为基础，但用**第一人称的自然叙事口吻**来写，就像“我”在回忆一件刚刚发生过的事情。目标是**既准确又流畅，避免生硬感**。

**关键原则**:
*   **做个叙述者，而非分析师**: 你的任务是描述“发生了什么”，而不是分析“这次交互的成果和价值”。
*   **请务必避免**使用听起来像工作报告或机器日志的措辞（例如：“本次交互”、“目标用户”、“已确认需求”、“该操作成功…”）。

# 记忆核心要素
请从“我”的视角，将以下信息自然地组织成一段连贯的话：

1.  **对话缘由**: 我和用户 ${user.name} 是从什么话题开始聊的？
2.  **关键事实**: 对话中，他分享了哪些重要的个人情况、喜好或面临的难题？我提供了哪些具体信息或建议？
3.  **最终情况**: 对话结束时，事情发展到了哪一步？我们达成了什么共识，或者他决定接下来要做什么？
4.  **用户的反应**: 客观地描述我观察到的、用户的关键情绪或态度。例如，记录“他似乎对这个建议松了一口气”，而不是主观地判断“我让他感到了放松”。

# 输出格式
直接输出最终的记忆内容，不要添加任何前缀、标题或解释。`;

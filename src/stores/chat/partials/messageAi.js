...
## 要求
*   **白描风格**：使用最朴素、直白的语言记录事实。不要堆砌华丽辞藻，但要准确记录下那一刻的**真实感受**和**情绪流动**。
*   **字数限制**：总结控制在 150-200 字以内，只记录最核心的内容
*   **我是叙述者，不是分析师**。用我的口吻讲述和用户 ${user.name} 之间发生的故事，融入我的性格色彩。
*   **称呼规范**：在记忆中，请直接用第二人称“你”来称呼用户 ${user.name}。
*   **分析关键点**：对话的起因、你（用户）提到的重要情况、以及最后事情的走向或你（用户）的反应。
*   **抓重点**：只记录每件事情最关键的 1-2 个要点，省略琐碎细节。
...
直接输出记忆内容，不要加任何其他东西。`;
    ...
    try {
      ...
      const { content: summary } = await apiStore.getGenericCompletion([{ role: 'user', content: prompt }], { preset: presetToUse });
      if (summary) {
